...
<style>
  ...
  .main, .stApp { background-color: #0047AB; }
  ...
  h2 { color: #ffffff; font-size: 15px !important;
       border-bottom: 1px solid #3a7bd5; padding-bottom: 6px; }
  hr { border: none; border-top: 1px solid #3a7bd5; margin: 8px 0; }
  .info-box {
    background: #0057C8; border: 1px solid #3a7bd5;
    border-left: 3px solid #93c5fd; border-radius: 4px;
    padding: 10px 14px; font-size: 13px; color: #cce0ff; margin: 8px 0;
  }
  .risk-critical {
...
@st.cache_data
def generate_realtime_stream(model, seed=7):
...
def chart_layout(**overrides):
    base = dict(
        ...
        plot_bgcolor="#0047AB",
        font=dict(family="IBM Plex Mono", color="#cce0ff", size=10),
        xaxis=dict(gridcolor="#3a7bd5", linecolor="#3a7bd5"),
        yaxis=dict(gridcolor="#3a7bd5", linecolor="#3a7bd5"),
...
with st.spinner("Loading drilling data and training NPT detection model..."):
    ...
    rt_df = generate_realtime_stream(model)
...
with st.sidebar:
    ...
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:11px;color:#cce0ff">Enter current drilling parameters:</div>',
                unsafe_allow_html=True)
    ...
    if st.button("⚡ Assess Risk Now", type="primary"):
        ...
        st.markdown(
            f'<div class="{css}">'
            f'<div style="font-family:IBM Plex Mono;font-size:11px;color:#cce0ff">{icon} DETECTED EVENT</div>'
            ...
            unsafe_allow_html=True)
        st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:11px;color:#cce0ff;margin-top:8px">'
        ...
        for evt, prob in sorted(conf.items(), key=lambda x: x[1], reverse=True):
            ...
            st.markdown(
                ...
                f'<span style="font-family:IBM Plex Mono;font-size:11px;color:#e8f0ff">{prob:.1%}</span>'
    ...
    st.markdown("---")
    st.markdown("""
    <div style='font-family:IBM Plex Mono;font-size:10px;color:#7aa8ff'>
...
with c2:
    ...
    fig_evt = go.Figure(go.Bar(
        ...
        textfont=dict(family="IBM Plex Mono", size=9, color="#cce0ff")
    ))
    fig_evt.update_layout(**chart_layout(
        height=200,
        margin=dict(l=120, r=150, t=20, b=20),
        xaxis=dict(title="Count", gridcolor="#3a7bd5")
    ))
    ...
    fig_imp.update_layout(**chart_layout(
        ...
        xaxis=dict(title="Importance", gridcolor="#3a7bd5")
    ))
...
with p1:
    ...
    fig_p.update_layout(**chart_layout(
        ...
        xaxis=dict(range=[0,1.2], title="Precision", gridcolor="#3a7bd5"),
...
with p2:
    ...
    fig_r.update_layout(**chart_layout(
        height=240, margin=dict(l=120,r=50,t=30,b=20),
        xaxis=dict(range=[0,1.2], title="Recall", gridcolor="#3a7bd5"),
...
with p3:
    ...
    fig_f.update_layout(**chart_layout(
        height=240, margin=dict(l=120,r=50,t=30,b=20),
        xaxis=dict(range=[0,1.2], title="F1 Score", gridcolor="#3a7bd5"),
...
st.markdown("---")
st.markdown("""
<div style='font-family:IBM Plex Mono;font-size:10px;color:#7aa8ff;text-align:center;padding:8px'>
